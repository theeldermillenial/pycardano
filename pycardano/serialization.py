...
def _loads_shape_scan(payload: bytes, **kwargs: Any) -> Any:
    """Decode CBOR while preserving order and indefinite-length arrays.

    Fallback for a cbor2 without ``array_hook``: decode, then re-scan the payload
    to recover which arrays were indefinite-length.
    """
...
def _build_decode_plan(t: typing.Type) -> Callable[[Any], Any]:
    ...
    if has_origin and origin is list:
        ...
        def plan_list(v, _sub=sub_plan):
            # Tolerate the ``tuple`` cbor2 6.x yields for arrays nested in a CBORTag
            # value (immutable decode); the original feat/cbor2 path normalized it to a
            # ``list``. IndefiniteList is a list subclass and is preserved as-is.
            if not isinstance(v, (list, tuple, IndefiniteList)):
                raise DeserializeException(f"Expected type list but got {type(v)}")
            restored = [_sub(w) for w in v]
    ...
    if has_origin and origin is dict:
        ...
        def plan_dict(v, _kp=key_plan, _vp=val_plan):
            # Tolerate the cbor2 6.x ``frozendict`` for maps nested in a CBORTag value.
